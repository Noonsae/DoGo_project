  ...
  try {
    ...
    if (!action || !userId || !hotelId) {
      return NextResponse.json(
        { error: "필수 값이 누락되었습니다. (action, userId, hotelId 필요)" },
        { status: 400 }
      );
    }
    ...
    if (action === "status") {
      ...
      if (error) {
        ...
        return NextResponse.json(
          { error: error.message || "즐겨찾기 조회 중 오류 발생" },
          { status: 500 }
        );
      }
    ...
    if (action === "add") {
      ...
      if (existingError) {
        ...
        return NextResponse.json(
          { error: existingError.message || "즐겨찾기 조회 중 오류 발생" },
          { status: 500 }
        );
      }
      ...
      if (error) {
        ...
        return NextResponse.json(
          { error: error.message || "즐겨찾기 추가 중 오류 발생" },
          { status: 500 }
        );
      }
    ...
    if (action === "remove") {
      ...
      if (error) {
        ...
        return NextResponse.json(
          { error: error.message || "즐겨찾기 삭제 중 오류 발생" },
          { status: 500 }
        );
      }
    ...
    return NextResponse.json(
      { error: "Invalid action (status, add 또는 remove를 사용하세요)" },
      { status: 400 }
    );
  } catch (error: any) {
...
export async function GET(req: Request) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: error.message || "즐겨찾기 데이터를 가져오는 중 오류 발생" },
        { status: 500 }
      );
    }
